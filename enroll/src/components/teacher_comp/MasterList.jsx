        ...
        <div className="studentList">
          {loading ? (
            ...
            <table className="masterlist-table">
              ...
              <tbody>
                {Array.from({ length: Math.max(boys.length, girls.length) }).map((_, i) => (
                  <tr key={i}>
                    <td className="column1">{boys[i] ? i + 1 : ''}</td>
                    <td>{boys[i] ? `${boys[i].last_name?.toUpperCase() || ''}, ${boys[i].first_name || ''}` : ''}</td>
                    <td className="column1">{girls[i] ? i + 1 : ''}</td>
                    <td>{girls[i] ? `${girls[i].last_name?.toUpperCase() || ''}, ${girls[i].first_name || ''}` : ''}</td>
                  </tr>
                ))}
              </tbody>
            </table>
